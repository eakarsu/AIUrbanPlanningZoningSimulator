...
export default function CfPublicCommentModerationPipelineWithSentimentAnalysis() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-public-comment-moderation-pipeline-with-sentiment-analysis/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-public-comment-moderation-pipeline-with-sentiment-analysis', project: 'AIUrbanPlanningZoningSimulator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-public-comment-moderation-pipeline-with-sentiment-analysis/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Public-comment moderation pipeline with sentiment analysis</h1>
