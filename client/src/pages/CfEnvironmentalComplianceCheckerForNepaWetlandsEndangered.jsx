...
export default function CfEnvironmentalComplianceCheckerForNepaWetlandsEndangered() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-environmental-compliance-checker-for-nepa-wetlands-endangered-species/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-environmental-compliance-checker-for-nepa-wetlands-endangered-species', project: 'AIUrbanPlanningZoningSimulator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-environmental-compliance-checker-for-nepa-wetlands-endangered-species/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Environmental compliance checker for NEPA/wetlands/endangered species</h1>
