...
export default function CfZoningScenarioOptimizerMaximizingAffordableHousingCommercial() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-zoning-scenario-optimizer-maximizing-affordable-housing-commercial-green-space/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-zoning-scenario-optimizer-maximizing-affordable-housing-commercial-green-space', project: 'AIUrbanPlanningZoningSimulator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-zoning-scenario-optimizer-maximizing-affordable-housing-commercial-green-space/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Zoning scenario optimizer maximizing affordable housing/commercial/green-space</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIUrbanPlanningZoningSimulator</p>
      </div>
