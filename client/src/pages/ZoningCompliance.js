...
const ENDPOINT = '/zoning-compliance';
...
const columns = [
  ...
  { key: 'proposedUse', label: 'Proposed Use' },
  { key: 'buildingHeight', label: 'Height (ft)' },
...
const formFields = [
  ...
  { key: 'zoneType', label: 'Zone Type', type: 'select', options: ['residential', 'commercial', 'industrial', 'mixed-use', 'agricultural'] },
  { key: 'currentUse', label: 'Current Use', type: 'text' },
  { key: 'proposedUse', label: 'Proposed Use', type: 'text' },
  { key: 'buildingHeight', label: 'Building Height (ft)', type: 'number' },
  { key: 'lotCoverage', label: 'Lot Coverage (%)', type: 'number' },
  { key: 'setback', label: 'Setback (ft)', type: 'number' },
...
const detailFields = [
  ...
  { key: 'lotCoverage', label: 'Lot Coverage (%)' },
  { key: 'setback', label: 'Setback (ft)' },
...
const ZoningCompliance = () => {
  ...
  return (
    <div className="page">
      <Navbar currentPage="Zoning Compliance" />
      <div className="page-content">
        <div className="page-header">
          <div className="page-header-left">
            <h1><FaMapMarkerAlt style={{ marginRight: '0.5rem', color: '#059669' }} />Zoning Compliance</h1>
            <p>Automated zoning regulation compliance checking</p>
          </div>
        ...
        {showForm && (
          <FormModal fields={formFields} item={editItem} onClose={() => { setShowForm(false); setEditItem(null); }}
            onSave={handleSave} title={editItem ? 'Edit Zoning Compliance' : 'New Zoning Compliance'} />
        )}
      </div>
    </div>
  );
...
export default ZoningCompliance;
