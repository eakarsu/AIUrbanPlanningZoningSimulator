...
export default function CfInfrastructureImpactPredictorForSchoolsUtilitiesTransit() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-infrastructure-impact-predictor-for-schools-utilities-transit/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-infrastructure-impact-predictor-for-schools-utilities-transit', project: 'AIUrbanPlanningZoningSimulator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-infrastructure-impact-predictor-for-schools-utilities-transit/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Infrastructure impact predictor for schools/utilities/transit</h1>
