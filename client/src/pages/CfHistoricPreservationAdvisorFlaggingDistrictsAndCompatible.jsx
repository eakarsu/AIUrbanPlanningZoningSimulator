...
export default function CfHistoricPreservationAdvisorFlaggingDistrictsAndCompatible() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-historic-preservation-advisor-flagging-districts-and-compatible-development/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-historic-preservation-advisor-flagging-districts-and-compatible-development', project: 'AIUrbanPlanningZoningSimulator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-historic-preservation-advisor-flagging-districts-and-compatible-development/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Historic preservation advisor flagging districts and compatible development</h1>
