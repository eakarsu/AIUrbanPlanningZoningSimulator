...
export default function GapCriticalOnly1AiEndpointDespiteScenario() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-only-1-ai-endpoint-despite-scenario-compliance/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-critical-only-1-ai-endpoint-despite-scenario-compliance', project: 'AIUrbanPlanningZoningSimulator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-only-1-ai-endpoint-despite-scenario-compliance/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Critical: Only 1 AI endpoint despite scenario, compliance, impact-assessment use cases</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIUrbanPlanningZoningSimulator</p>
      </div>
