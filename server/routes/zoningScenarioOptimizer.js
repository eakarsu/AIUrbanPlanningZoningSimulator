...
// Zoning scenario optimizer maximizing affordable housing/commercial/green-space
// Feature: zoning-scenario-optimizer
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Zoning scenario optimizer maximizing affordable housing/commercial/green-space.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'zoning-scenario-optimizer', payload, result);
    res.json({ success: true, feature: 'zoning-scenario-optimizer', ...result });
  } catch (err) {
    console.error('zoning-scenario-optimizer analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Zoning scenario optimizer maximizing affordable housing/commercial/green-space. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'zoning-scenario-optimizer', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'zoning-scenario-optimizer', title: 'Zoning scenario optimizer maximizing affordable housing/commercial/green-space', project: 'AIUrbanPlanningZoningSimulator' });
});
