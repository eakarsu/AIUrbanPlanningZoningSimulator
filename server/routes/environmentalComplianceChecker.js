...
// Environmental compliance checker for NEPA/wetlands/endangered species
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Environmental compliance checker for NEPA/wetlands/endangered species.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'environmental-compliance-checker', payload, result);
    res.json({ success: true, feature: 'environmental-compliance-checker', ...result });
  } catch (err) {
    console.error('environmental-compliance-checker analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Environmental compliance checker for NEPA/wetlands/endangered species. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'environmental-compliance-checker', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'environmental-compliance-checker']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'environmental-compliance-checker', title: 'Environmental compliance checker for NEPA/wetlands/endangered species', project: 'AIUrbanPlanningZoningSimulator' });
});
