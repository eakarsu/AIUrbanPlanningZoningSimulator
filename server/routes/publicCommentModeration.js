...
// Public-comment moderation pipeline with sentiment analysis
// Feature: public-comment-moderation
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Public-comment moderation pipeline with sentiment analysis.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'public-comment-moderation', payload, result);
    res.json({ success: true, feature: 'public-comment-moderation', ...result });
  } catch (err) {
    console.error('public-comment-moderation analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Public-comment moderation pipeline with sentiment analysis. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'public-comment-moderation', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'public-comment-moderation', title: 'Public-comment moderation pipeline with sentiment analysis', project: 'AIUrbanPlanningZoningSimulator' });
});
