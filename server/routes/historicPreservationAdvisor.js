...
// Historic preservation advisor flagging districts and compatible development
// Feature: historic-preservation-advisor
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIUrbanPlanningZoningSimulator" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Historic preservation advisor flagging districts and compatible development.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'historic-preservation-advisor', payload, result);
    res.json({ success: true, feature: 'historic-preservation-advisor', ...result });
  } catch (err) {
    console.error('historic-preservation-advisor analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIUrbanPlanningZoningSimulator". Return strict JSON only.',
      `Feature: Historic preservation advisor flagging districts and compatible development. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'historic-preservation-advisor', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'historic-preservation-advisor']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'historic-preservation-advisor', title: 'Historic preservation advisor flagging districts and compatible development', project: 'AIUrbanPlanningZoningSimulator' });
});
