...
// Community benefit analyzer quantifying jobs/tax/housing impact
// Feature: community-benefit-analyzer
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Community benefit analyzer quantifying jobs/tax/housing impact.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'community-benefit-analyzer', payload, result);
    res.json({ success: true, feature: 'community-benefit-analyzer', ...result });
  } catch (err) {
    console.error('community-benefit-analyzer analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Community benefit analyzer quantifying jobs/tax/housing impact. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'community-benefit-analyzer', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'community-benefit-analyzer']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'community-benefit-analyzer', title: 'Community benefit analyzer quantifying jobs/tax/housing impact', project: 'AIUrbanPlanningZoningSimulator' });
});
