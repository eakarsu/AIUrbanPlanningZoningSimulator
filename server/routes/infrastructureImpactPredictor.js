...
// Infrastructure impact predictor for schools/utilities/transit
// Feature: infrastructure-impact-predictor
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Infrastructure impact predictor for schools/utilities/transit.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'infrastructure-impact-predictor', payload, result);
    res.json({ success: true, feature: 'infrastructure-impact-predictor', ...result });
  } catch (err) {
    console.error('infrastructure-impact-predictor analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Infrastructure impact predictor for schools/utilities/transit. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'infrastructure-impact-predictor', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'infrastructure-impact-predictor']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'infrastructure-impact-predictor', title: 'Infrastructure impact predictor for schools/utilities/transit', project: 'AIUrbanPlanningZoningSimulator' });
});
